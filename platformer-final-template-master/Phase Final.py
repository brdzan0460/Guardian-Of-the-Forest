...
class Hero(pygame.sprite.Sprite):
    ...
    def can_slam(self, tiles):
        return not self.can_jump(tiles)

    def slam(self, tiles):
        if self.can_slam(tiles):
            self.terminal = False
            self.vy = 70
...
class Game():
    ...
    def calculate_offset(self):            
        ...
        x = self.offset_x + displacement/16
    ...
    def render(self):
        ...
        self.offset_x, offset_y = self.calculate_offset()
        bg1_offset_x = -1 * self.offset_x * self.level.parallax_speed1
        bg1_offset_y = -1 * offset_y * self.level.parallax_speed1
        bg2_offset_x = -1 * self.offset_x * self.level.parallax_speed2
        ...
        screen.blit(self.level.world, [self.offset_x, offset_y])
